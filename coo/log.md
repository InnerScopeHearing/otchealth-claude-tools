...
2026-06-17 EVENT | GitHub secret scan alert - InnerScopeHearing | escalated | EVENT MODE. Inbound email from support@github.com: 8 secrets detected across 18 InnerScopeHearing repos in historical scan. This escalates the existing HARD GATE (GCP SA + PostHog already flagged as compromised). Actions taken: (1) Created urgent COO Task in Notion (Priority: Now, Approval: Needs Matt, Cash Lever: Compliance) with full remediation steps. (2) Sent alert email to matthew@innd.com via coo@innd.com with exact steps: view alerts at GitHub Security tab, revoke/rotate all 8 secrets, purge git history with BFG after rotation. Gate: NO investor-facing action (Reg D, data room, WeFunder) until all 8 confirmed rotated and GitHub alerts closed.
