...
2026-06-10 18:02 | send-email bug | found+fixed | COO email "Got it - picture updated, here is what changed" (execution 5518) sent blank body. Root cause: payload used field name "html" but the Send Email workflow reads "$json.body.body". Fix: resent with correct field name (execution 5527). Bug note: any future COO email call must use "body" not "html" as the payload field.
2026-06-10 18:xx | event-mode | handled | Matt replied to blank email. EVENT MODE: diagnosed delivery bug, resent corrected version, logged root cause. No morning brief re-sent (morning brief already out, marker confirmed).
