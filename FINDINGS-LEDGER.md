...
### finding:FND-20260724-cd01 severity:critical status:open | Xero rejects ANY ManualJournal write (new post or void) that references an ARCHIVED account - blocked voiding QBO-JE-18471, leaving InnerScope's HA Investment-in-Subsidiary DOUBLE-COUNTED at 31,758,284 (both the old 15,879,142 lump entry on archived acct 1808 and the new 15,879,142 detail on active acct 1740 are simultaneously live) as of this finding. Confirmed via controlled A/B test: identical 1 dollar journal succeeds against active account 1740, fails identically against archived account 1808. Also confirms this is a DIFFERENT root cause than the earlier accounting.settings scope finding (FND-20260724-68f5) - this fails even though ManualJournals writes are otherwise fully working (14 other new journals posted successfully same session).

- **Source audit doc:** FY2021 Completion Punch List Item 11 (HA acquisition) - live posting session 2026-07-23/24
- **Fix commit:** (none yet)
- **Verified by:** Hyperagent CFO seat 2026-07-24: reproduced the QBO-JE-18471 void failure twice, then a reversing-entry attempt referencing the same archived account failed identically, then ran a controlled isolation test (identical $1 journal, only the account code changed) that succeeded against 1740 and failed against 1808 - conclusive proof of the archived-account cause, not a payload or scope issue. Diagnostic test journal was cleaned up (voided, independently verified). Current state left as-is (double-counted) pending resolution - either (a) the CTO adds accounting.settings scope so 1808 can be un-archived, reversed, and re-archived, or (b) a human with direct Xero UI access voids QBO-JE-18471 or un-archives 1808 manually (UI may not enforce the same API scope restriction). Flagged to Matt directly, not silently left for a future session to rediscover.
...
### finding:FND-20260724-b55e severity:high status:open | The gateway's Xero OAuth connection (cfo lane) is missing the accounting.settings WRITE scope: it has accounting.transactions + accounting.settings.read but not accounting.settings. This blocks ALL Xero chart-of-accounts / organisation-settings writes - confirmed on POST /Accounts (create new account, e.g. 6573 Stock Based Comp) and POST /Accounts/{id} (un-archive account 1808). This is the ROOT CAUSE that makes the HA double-count unfixable via API: the only API path to remove the old lump entry QBO-JE-18471 is un-archive 1808 -> void the journal -> re-archive, and the un-archive step needs this scope. Transaction writes (BankTransactions, ManualJournals, Invoices, Contacts, etc.) are unaffected and work fine.

- **Source audit doc:** OTCHealth Gateway / Xero OAuth app - cfo lane scope configuration (diagnosed 2026-07-24)
- **Fix commit:** (none yet)
- **Verified by:** Hyperagent CFO seat 2026-07-24: diagnosed via 2 independent paths that converged - (1) web research confirmed Xero's OpenAPI spec gates POST/PUT /Accounts behind accounting.settings, a different scope from accounting.transactions which covers the working transaction endpoints; (2) live read-only diagnostics confirmed GET /Accounts works (settings.read present, 420 accounts read) while every POST /Accounts variant fails with a generic ValidationException, and the xero_request tool's own description omits /Accounts from its example writable paths. FIX: add accounting.settings scope to the gateway's Xero app registration and re-consent all 4 orgs. Keep it on the gateway's single Cosmos-based token chain - do NOT revive the retired standalone Xero Accounting API skill's separate chain (that recreates the single-use-token race). Currently documented in the OTCHealth Gateway (CFO) skill KNOWN ISSUE section; this finding gives it a trackable ID.
