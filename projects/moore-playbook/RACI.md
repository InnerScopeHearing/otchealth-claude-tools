...
|---|---|---|
| **OTCHealth Inc.** (operating company, the Reg D/CF/A+ issuer) | **The PSAP (not an app, not an OTC hearing aid):** iHEAR TReO. **The OTC hearing aid line (regulated, FDA-registration-gated):** iHEAR Matrix, EAZE Classic, STREAM RIC, CONNECT ITE (legacy, currently unavailable), iHEAR Linx, iHEAR Axis (pre-launch). **Apps:** iHEARtest, AWARE, InnerEase, OTCHealth Companion, MedReview (PHI-walled). **Commerce/sales infra:** the Shopify store (otchealthmart.com), the AI voice sales channel (Promo Sarah/Main Sarah/Helen). **Future/internal-only, counsel-blocked (17b):** CareNow/SaveRx. | InnerScope's own public-company financials/stock; the personal-project apps below |
