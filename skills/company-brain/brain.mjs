...
let AIS_EP, AIS_KEY, AOAI_EP, AOAI_KEY, AOAI_DEP, CHAT_EP, CHAT_KEY, CHAT_DEP;
async function init() {
  ...
  CHAT_EP = (await sm("azure-openai-endpoint") || "").replace(/\/$/, ""); CHAT_KEY = await sm("azure-openai-key"); CHAT_DEP = process.env.BRAIN_MODEL || "gpt-4o";
...
async function chat(system, user) { const r = await fetch(`${CHAT_EP}/openai/deployments/${CHAT_DEP}/chat/completions?api-version=2024-06-01`, { method: "POST", headers: { "api-key": CHAT_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ messages: [{ role: "system", content: system }, { role: "user", content: user }], max_tokens: 900, temperature: 0.2 }) }); if (!r.ok) throw new Error("chat " + r.status); return (await r.json()).choices[0].message.content; }
