...
(async()=>{
  try{
    ...
    // claude-driver SA is actually present. Same secret names either way.
    let apiKey=await kvSecret("datadog-api-key"), site=await kvSecret("datadog-site");
    if(!apiKey){
      const sa=loadSA();
      if(sa){ const tok=await gcpToken(sa); if(tok){ apiKey=apiKey||await sm(tok,"datadog-api-key"); site=site||await sm(tok,"datadog-site"); } }
    }
    if(!apiKey) return;
