...
// 4) Azure OpenAI — optional (opt-in; models pending quota)
if (creds.azureOpenAIEndpoint && creds.azureOpenAIKey) {
    const base = creds.azureOpenAIEndpoint.replace(/\/+$/, '');
    const r = await timed(() => fetch(`${base}/openai/models?api-version=${creds.azureOpenAIApiVersion}`, { headers: { 'api-key': creds.azureOpenAIKey } }));
    if (r._err) rec('Azure OpenAI', 'FAIL', r._err, false);
    else if (r.ok) rec('Azure OpenAI', 'PASS', `key valid @ ${base}`, false);
    else rec('Azure OpenAI', 'FAIL', `HTTP ${r.status} — key invalid or wrong endpoint (fix azure-openai-key)`, false);
