...
// (1) the default backend is now s3, behaviorally, (2) a room with no verified S3 mirror row fails
// LOUD before any network call rather than guessing a bucket, and (3) --azure remains explicitly
// selectable for read-only inspection and is never silently rewritten.
//
// FIXTURE NOTE (2026-08-19). These tests used the `commerce` profile as their canonical UNMAPPED
// room. Commerce has since been mapped (its bucket was observed by a live listing and a MIRROR row
// added), which broke three of them -- the BEHAVIOR was still correct, only the example was stale.
// They now use an explicitly fictional container instead of a real profile, so the fixture cannot
// be invalidated again by legitimately mapping another room. A separate test below pins that
// commerce IS mapped, so the two facts stay in agreement rather than drifting apart silently.
//
...
// A container name that is deliberately fictional, so no future MIRROR addition can ever make this
// fixture legitimately mapped and silently turn the test green for the wrong reason.
const UNMAPPED = ["--azure-account", "otchealthcfodata", "--container", "no-such-room-fixture"];

test("DEFAULT (no --s3/--azure/--gcs flag) resolves to the S3 backend -- an unmapped room fails with the S3-specific 'no mirror mapping' error, not an Azure/GCS one, proving the default is s3", () => {
  const r = run(["status", "--profile", "finance", ...UNMAPPED]);
  assert.notEqual(r.status, 0, "an unmapped room must not silently succeed under the s3 default");
  assert.equal(r.status, 2);
  assert.match(r.stderr, /no S3 mirror mapping for otchealthcfodata\/no-such-room-fixture/);
});

test("--azure explicitly selects the Azure path for the SAME unmapped room, bypassing the S3 default entirely", () => {
  ...
  const r = run(["status", "--profile", "finance", ...UNMAPPED, "--azure"]);
...
test("STORAGE_BACKEND=s3 env var (no flag) resolves the same way as the default", () => {
  let threw = null;
  try {
    execFileSync("node", [INDEXER_MJS, "status", "--profile", "finance", ...UNMAPPED], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, STORAGE_BACKEND: "s3" } });
  } catch (e) { threw = e; }
  assert.ok(threw, "an unmapped room under STORAGE_BACKEND=s3 must not succeed (no mirror row)");
  assert.equal(threw.status, 2);
  assert.match(threw.stderr, /no S3 mirror mapping/);
});

test("the finance profile (a VERIFIED S3 mirror room) passes the mapping check under the s3 default -- never the 'no mirror mapping' refusal an unmapped room gets", () => {
...
test("a write failure (an unmapped room forced onto --s3 explicitly) exits non-zero with a diagnosable message, never silently swallowed", () => {
  const r = run(["index", "--profile", "finance", ...UNMAPPED, "--s3", "--limit", "1"]);
...
// Pins the 2026-08-19 commerce mapping, so the fixture change above and the real MIRROR table
// cannot drift apart: if commerce were ever unmapped again, this fails rather than the tests above
// quietly going back to testing the thing they were originally written against.
test("commerce IS mapped (its bucket was observed live 2026-08-19) and no longer behaves as an unmapped room", () => {
  const r = run(["status", "--profile", "commerce"]);
  assert.doesNotMatch(r.stderr, /no S3 mirror mapping/, "commerce has a verified S3 mirror row and must not be refused as unmapped");
});
