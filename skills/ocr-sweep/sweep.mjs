...
(async()=>{
  ...
  const stores=[];
  if(want.includes("legal")){ const k=await sm(g,"azure-legal-storage-key"); if(k) stores.push({name:"legal",account:"otchealthlegalstore",key:k,containers:["company","personal"]}); else console.error("[ocr-sweep] WARN: azure-legal-storage-key unavailable — skipping legal store this run."); }
