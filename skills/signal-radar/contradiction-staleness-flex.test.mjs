// Tests for skills/signal-radar/detectors/contradiction-staleness.mjs's FLEX PROCESSING adoption
// (2026-08-29, see setup/model-routing.mjs's own header for the full OpenAI service_tier contract).
// Mirrors the withStubbedFetch/withEnv convention of the sibling
// contradiction-staleness-openai-port.test.mjs.
//
// CONTRADICTION_TIER is resolved ONCE at module-load time (a top-level const), so every test here
// uses a cache-busting dynamic import to get a fresh module instance that sees the env set BEFORE
// import, exactly like setup/model-routing.test.mjs's freshImport() pattern.
import { test } from "node:test";
import assert from "node:assert/strict";

async function freshContradiction() {
...
const NEW_ROW = { id: "20260829-002", type: "fact", ts: "2026-08-29T00:00:00Z", text: "flatstick build is CFBundleVersion 26", ekeys: ["flatstick"] };
const SLICE = [{ id: "20260819-001", type: "fact", ts: "2026-08-19T00:00:00Z", text: "flatstick build is CFBundleVersion 25" }];
...
test("DEFAULT (OPENAI_SERVICE_TIER* unset): no service_tier, no AbortSignal, single attempt -- byte-identical to before the flex lane existed", async () =>
  withEnv(NO_CREDS_ENV, async () => {
    const { makeEntailer } = await freshContradiction();
    ...
    const verdict = await withEnv({ OPENAI_API_KEY: "sk-test-fake-not-real" }, () =>
      withStubbedFetch(async (url, opts) => {
        ...
        return { ok: true, status: 200, headers: new Map(), json: async () => ({ choices: [{ message: { content: JSON.stringify({ label: "supersede", citedId: SLICE[0].id }) } }] }) };
      }, async () => { const entailer = await makeEntailer(); return entailer(NEW_ROW, SLICE); }));
    assert.equal(verdict.label, "supersede");
    assert.equal(calls, 1);
    assert.equal("service_tier" in captured.body, false);
    assert.equal(captured.signal, undefined);
  }));

test("OPENAI_SERVICE_TIER_SIGNAL_RADAR_CONTRADICTION_STALENESS=flex: adds service_tier, an AbortSignal, and retries a 429 with backoff", async () =>
  withEnv({ ...NO_CREDS_ENV, OPENAI_SERVICE_TIER_SIGNAL_RADAR_CONTRADICTION_STALENESS: "flex" }, async () => {
    const { makeEntailer } = await freshContradiction();
    ...
    await withEnv({ OPENAI_API_KEY: "sk-test-fake-not-real" }, () =>
      withStubbedFetch(async () => { calls++; return { ok: false, status: 500, headers: new Map(), json: async () => ({}) }; },
        async () => {
          ...
          await assert.rejects(() => entailer(NEW_ROW, SLICE), (e) => { assert.equal(e.message, "detector ERROR: OpenAI entailment call failed: chat 500"); return true; });
        }));
