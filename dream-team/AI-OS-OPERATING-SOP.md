...
- **agent-evals.** Golden-task harness: runs a role persona, scores it with an LLM-judge against an explicit rubric, prints a scorecard, and with `--emit` sends `eval_result` events to the PostHog Fleet Agents project (479484). CI-gateable; catches per-role quality regressions. Companion drift monitors: `recall-evals` / `embedding-drift-monitor` measure whether memory recall is silently degrading. `recall-evals` is now a HARD PAGER, not just a dashboard (W1-4, 2026-07-17): the old "0.333 baseline" was an artifact of two bugs (a golden-set `agent` filter mismatch + a line-vs-hit scoring miscalibration, both fixed) — the corrected, freshly-mined 43-item set genuinely scores hit@5 = 1.0 (`skills/recall-evals/baseline.json`), and the nightly workflow now runs `--strict` (no `continue-on-error`), so a real regression past the baseline SLO fails the run instead of sitting in PostHog unread.
...
1. **Confirm the maintenance layer is green.** azure-canary is not paging (no stale index, no failed job, no stale PostHog telemetry stream); fleet-medic shows no memory-off agent on your lane.
2. **Run / review your role eval.** `agent-evals` scorecard for your role; check recall-evals / embedding-drift hit@5 has not regressed below the recorded baseline (`skills/recall-evals/baseline.json`; nightly-recall-eval.yml now pages on its own if it has).
