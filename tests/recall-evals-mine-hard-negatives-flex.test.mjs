// Tests for skills/recall-evals/mine-hard-negatives.mjs's FLEX PROCESSING adoption (2026-08-29, see
// setup/model-routing.mjs's own header for the full OpenAI service_tier contract).
//
// mine-hard-negatives.mjs had NO retry-on-429 at all before this change (a bare `if (!r.ok) throw`) --
// the "default behavior lock" here is therefore that a single 429 (or any other non-2xx) throws
// IMMEDIATELY with the exact original `chat <status>: <body>` message shape, never retried, unless
// OPENAI_SERVICE_TIER(_RECALL_EVALS_MINE_HARD_NEGATIVES) is set. This file is already imported
// directly by tests/recall-evals-hard-negative-mining.test.mjs (it already had an isMain guard), so
// no test-safety refactor was needed here -- only the flex-lane behavior is new.
...
test("DEFAULT (OPENAI_SERVICE_TIER* unset): a single attempt, no service_tier, no AbortSignal -- byte-identical to the pre-existing zero-retry behavior", async () =>
  withEnv({ ...NO_CREDS_ENV, OPENAI_API_KEY: "sk-test-fake-not-real" }, async () => {
    ...
    // 500 -> 2000 (2026-08-30, FND-20260830-e927): see MINE_HARDNEG_MAX_TOKENS's own comment in
    // mine-hard-negatives.mjs and tests/recall-evals-mine-hard-negatives-truncation.test.mjs for the
    // reasoning-truncation fix this budget bump is part of.
...
test("OPENAI_SERVICE_TIER_RECALL_EVALS_MINE_HARD_NEGATIVES=flex: adds service_tier, an AbortSignal, and retries a 429 with backoff", async () =>
  withEnv({ ...NO_CREDS_ENV, OPENAI_API_KEY: "sk-test-fake-not-real", OPENAI_SERVICE_TIER_RECALL_EVALS_MINE_HARD_NEGATIVES: "flex" }, async () => {
    const { callChat } = await freshMineHardNeg();
